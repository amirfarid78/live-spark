...
export default function Agency() {
  ...
  const stats = [
    { label: "Active Agencies", value: "120+", icon: Building2 },
    { label: "Managed Creators", value: "5,000+", icon: Users },
    { label: "Total Payouts", value: "$2.5M+", icon: TrendingUp },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-background pb-24">
      <header className="sticky top-0 z-40 glass border-b border-border/50">
        <div className="flex items-center justify-between px-4 py-3">
          <h1 className="text-xl font-bold">Agency Program</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
        <TabsList className="w-full justify-around border-b border-border bg-transparent h-12 rounded-none px-2">
          ...
          <TabsTrigger value="apply" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent text-xs sm:text-sm">
            Apply Now
          </TabsTrigger>
          <TabsTrigger value="dashboard" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent text-xs sm:text-sm">
            Dashboard
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="mt-0 px-4 py-5 space-y-6">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-gradient-primary p-6 text-white">
            ...
            <Button onClick={() => setActiveTab("apply")} className="bg-white text-primary hover:bg-white/90 font-semibold rounded-xl">
        ...
        <TabsContent value="apply" className="mt-0 px-4 py-5 space-y-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input placeholder="Your agency name" value={form.agency_name} onChange={(e) => setForm({ ...form, agency_name: e.target.value })} className="mt-1.5 rounded-xl" />
            </div>
            <div>
              <Label>Description</Label>
              <Textarea placeholder="Tell us about your agency..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="mt-1.5 rounded-xl min-h-[100px]" />
            </div>
            <div>
              <Label>Experience</Label>
              <Textarea placeholder="Your experience managing creators..." value={form.experience} onChange={(e) => setForm({ ...form, experience: e.target.value })} className="mt-1.5 rounded-xl" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Creators You Manage</Label>
                <Input type="number" placeholder="0" value={form.creator_count || ""} onChange={(e) => setForm({ ...form, creator_count: parseInt(e.target.value) || 0 })} className="mt-1.5 rounded-xl" />
              </div>
              <div>
                <Label>Contact Email *</Label>
                <Input type="email" placeholder="email@agency.com" value={form.contact_email} onChange={(e) => setForm({ ...form, contact_email: e.target.value })} className="mt-1.5 rounded-xl" />
              </div>
            </div>
            <div>
              <Label>Phone (optional)</Label>
              <Input placeholder="+1 234 567 890" value={form.contact_phone} onChange={(e) => setForm({ ...form, contact_phone: e.target.value })} className="mt-1.5 rounded-xl" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Instagram</Label>
                <Input placeholder="@handle" value={form.instagram} onChange={(e) => setForm({ ...form, instagram: e.target.value })} className="mt-1.5 rounded-xl" />
              </div>
              <div>
                <Label>TikTok</Label>
                <Input placeholder="@handle" value={form.tiktok} onChange={(e) => setForm({ ...form, tiktok: e.target.value })} className="mt-1.5 rounded-xl" />
              </div>
            </div>
            <Button onClick={handleApply} disabled={applying} className="w-full bg-gradient-primary hover:opacity-90 rounded-xl h-12 font-semibold text-base">
        ...
        <TabsContent value="dashboard" className="mt-0 px-4 py-5">
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <Button onClick={() => setActiveTab("apply")} className="bg-gradient-primary hover:opacity-90 rounded-xl font-semibold">
